...
def gen_tasks_and_delays(agents, starts, goals, n_tasks, task_freq, n_delays_per_agent):
    arrival_time = 0
    tasks = []
    delays = {}
    ...
    for i in range(len(agents)):
        delays[agents[i]['name']] = random.sample(range(n_tasks * 10), n_delays_per_agent)
